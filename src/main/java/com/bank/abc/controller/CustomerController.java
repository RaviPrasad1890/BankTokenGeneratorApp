...
public class CustomerController {
	...
	public String retreiveCustomer(@ModelAttribute Customer cust1, ModelMap model) {
		...
		try {
			...
			model.put("error", "Please enter appropriate value");
			return "customer";
		...
		Customer cust = service.retrieveCustomer(id);
		if (cust == null) {
	...
	public String addCustomer(@RequestParam Map<String, String> formData, ModelMap map) {
		 ...
		 */
		char isPrimary;
		String custName = formData.get("custName");
		try {
			if ((formData.get("primaryCustomer").equalsIgnoreCase("on"))) {
				isPrimary = 'Y';
			} else {
				isPrimary = 'N';
			}
		...
		service.addCustomer(custName, isPrimary);
	...
	@RequestMapping(value = "/generate-customer-token", method = RequestMethod.GET)
	public String generateToken(@RequestParam String id, ModelMap model) {

		int custId = Integer.parseInt(id);
		Customer cust = service.retrieveCustomer(Integer.parseInt(id));

		Token tkn = cust.getToken();
		try {
			if (tkn.getActive() == 'Y') {
		...
		int tokenNumer = service.assignToken(custId);
		Counter counter = service.assignCounter(custId);

		Token token = new Token(tokenNumer, 'Y', counter);
		cust.setToken(token);
		String cno = Integer.toString(counter.getNumber());
		cust.setCounterAssigned(cno);
		model.put("cust", cust);
	...
	public String completeToken(@RequestParam Map<String, String> reqData) {
		// int tokenId= Integer.parseInt(reqData.get("number"));
		int custId = Integer.parseInt(reqData.get("custId"));
		service.markTokenComplete(custId);
